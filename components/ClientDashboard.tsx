...
import { ClientProfile, WeekProgress } from '../types';
import { WEEKLY_CONTENT, GUIDES } from '../constants';
import { Lock, CheckCircle, Play, FileText, Video, Headphones, ChevronRight, BarChart, LogOut, Calendar, Bell, Pause } from 'lucide-react';
...
export const ClientDashboard: React.FC<Props> = ({ user, onLogout }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800">
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 h-16 md:h-20 flex justify-between items-center">
          <div className="flex items-center gap-3 md:gap-4">
             ...
             <div className="hidden sm:block">
                ...
                <p className="text-[10px] text-brand-600 font-bold uppercase tracking-widest leading-none mt-1">Plataforma RFAI</p>
             </div>
          </div>
          <div className="flex items-center gap-3 md:gap-4">
      ...
      <main className="max-w-5xl mx-auto px-4 py-6 md:py-8 sm:px-6 lg:px-8">
        <div className="mb-8 md:mb-10 text-center md:text-left md:flex justify-between items-end">
          ...
          <div className="mt-4 md:mt-0">
             <div className="inline-flex items-center gap-2 bg-brand-50 px-3 py-1.5 md:px-4 md:py-2 rounded-full border border-brand-100 text-brand-700 text-xs md:text-sm font-semibold">
                <BarChart size={14} />
        ...
        <div className="relative space-y-6 md:space-y-8 before:absolute before:inset-0 before:ml-4 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-brand-200 before:via-slate-200 before:to-transparent">
          {[1, 2, 3, 4].map((weekNum) => {
            const config = WEEKLY_CONTENT[weekNum as keyof typeof WEEKLY_CONTENT];
              ...
              <div key={weekNum} className={`relative flex items-start md:items-center justify-between md:justify-normal md:odd:flex-row-reverse group ${isLocked ? 'opacity-60 grayscale' : 'opacity-100'}`}>
                ...
                <div className={`flex items-center justify-center w-8 h-8 md:w-10 md:h-10 rounded-full border-4 shadow shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 z-10 transition-all duration-300 mt-1 md:mt-0
                    ${isCompleted ? 'bg-brand-500 border-brand-100 text-white' : 
                      isCurrent ? 'bg-white border-brand-500 text-brand-600 scale-110 md:scale-125' : 
                ...
                <div className="w-[calc(100%-3rem)] md:w-[calc(50%-2.5rem)] ml-4 md:ml-0 p-5 md:p-6 bg-white rounded-2xl md:rounded-3xl shadow-sm border border-slate-100 transition-all hover:shadow-lg">
                    <div className="flex justify-between items-start mb-3 md:mb-4">
                        <div>
                            <span className="text-[10px] md:text-xs font-bold tracking-wider text-brand-600 uppercase mb-1 block">Semana {weekNum}</span>
                        ...
                        {isCurrent && <span className="px-2 py-0.5 md:px-3 md:py-1 bg-brand-100 text-brand-700 text-[10px] md:text-xs font-bold rounded-full animate-pulse whitespace-nowrap">En curso</span>}
                    </div>
                    ...
                    {!isLocked && (
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 md:gap-3">
                            <button onClick={() => setShowTest(true)} className="flex items-center gap-3 p-2 md:p-3 rounded-xl bg-slate-50 hover:bg-brand-50 border border-slate-100 hover:border-brand-200 transition-all group/btn text-left">
                                <div className="w-8 h-8 rounded-full bg-white flex items-center justify-center text-brand-600 shadow-sm"><FileText size={16} /></div>
                            ...
                            {config.hasMeet && (
                                <div className="relative group/meet">
                                    ...
                                    <a href="https://meet.google.com" target="_blank" rel="noreferrer" className="flex items-center gap-3 p-2 md:p-3 rounded-xl bg-indigo-50/50 hover:bg-indigo-50 border border-indigo-100 hover:border-indigo-200 transition-all group/btn text-left relative overflow-hidden h-full">
                                        <div className="w-8 h-8 shrink-0 rounded-full bg-white flex items-center justify-center text-indigo-600 shadow-sm z-10"><Video size={16} /></div>
                                        <div className="z-10 overflow-hidden"><span className="block text-sm font-bold text-indigo-900 truncate">Sesión</span><span className="text-[10px] text-indigo-600 font-medium truncate block">{getMeetingDate(user.startDate, weekNum)}</span></div>
                                    </a>
                                </div>
                            )}
                            ...
                            <div className="sm:col-span-2 mt-1 p-3 md:p-4 bg-slate-800 rounded-xl md:rounded-2xl text-white flex items-center justify-between shadow-lg shadow-slate-200">
                                <div className="flex items-center gap-3 overflow-hidden">
                                    <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-brand-500 flex items-center justify-center shrink-0"><Headphones size={16} /></div>
                                ...
                                <button onClick={() => setAudioPlaying(audioPlaying === weekNum ? null : weekNum)} className="w-8 h-8 md:w-10 md:h-10 shrink-0 rounded-full bg-white text-slate-900 flex items-center justify-center hover:bg-brand-400 transition-colors">
      ...
      {showGuide && <GuideModal week={user.currentWeek} steps={GUIDES[user.currentWeek] || []} onClose={() => setShowGuide(false)} onComplete={handleGuideComplete} />}
      {showTest && <TestModule onClose={() => setShowTest(false)} onComplete={handleTestComplete} />}
    </div>
  );
};